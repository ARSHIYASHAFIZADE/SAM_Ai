      ...
      <div style={DS.wrap}>
        ...
        {!result ? (
          ...
          <div>
            ...
            <PrintReport result={result} assessmentType="Breast Cancer" formFields={FIELDS.map(f => ({ label: (f as {label:string}).label, value: form[(f as {key:string}).key] ?? '', unit: (f as {unit?:string}).unit }))} />
          </div>
        )}
      </div>
    </div>
